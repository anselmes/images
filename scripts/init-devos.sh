...
# note: no riscv64
if [[ ! "riscv64" == "${ARCH}" ]]; then
  # install yq
  if [[ -z $(command -v yq) && ! "riscv64" == "${ARCH}" ]]; then
  ...
  # install buf
  if [[ -z $(command -v buf)  && ! "riscv64" == "${ARCH}" ]]; then
  ...
  # install cfssl
  if [[ -z $(command -v cfssl)  && ! "riscv64" == "${ARCH}"  ]]; then
  ...
  # install cilium cli
  if [[ -z $(command -v cilium)  && ! "riscv64" == "${ARCH}" ]]; then
  ...
  if [[ -z $(command -v sops) ]]; then
    curl -fsSLo /tmp/sops.deb "https://github.com/getsops/sops/releases/download/v${SOPS_VERSION}/sops_${SOPS_VERSION}_${ARCH}.deb"
    dpkg -i /tmp/sops.deb
  fi
