...
if grep -q -E "^omada:" /etc/group >/dev/null 2>&1; then
  ...
  if awk -F ':' '{print $3}' /etc/group | grep -q "^${PGID}$"; then
...
if id -u omada >/dev/null 2>&1; then
  ...
  if awk -F ':' '{print $3}' /etc/passwd | grep -q "^${PUID}$"; then
...
for ELEM in MANAGE_HTTP_PORT MANAGE_HTTPS_PORT PORTAL_HTTP_PORT PORTAL_HTTPS_PORT PORT_ADOPT_V1 PORT_APP_DISCOVERY PORT_UPGRADE_V1 PORT_MANAGER_V1 PORT_MANAGER_V2 PORT_DISCOVERY; do
...
for DIR in data logs properties; do
...
if [ -f "/cert/${SSL_KEY_NAME}" ] && [ -f "/cert/${SSL_CERT_NAME}" ]; then
  ...
  if [ -d /opt/tplink/EAPController/keystore ]; then
